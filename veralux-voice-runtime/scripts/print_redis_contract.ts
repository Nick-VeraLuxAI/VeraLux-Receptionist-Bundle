...
const reportLines = [
  ...
  "",
  "## Scope",
  "- Redis usage in `src/tenants/tenantResolver.ts`, `src/tenants/tenantConfig.ts`, `src/limits/capacity.ts`, and `src/tts/cache.ts` (TTS response cache).",
  "",
  ...
  "- TTS cache (optional): `TTS_CACHE_ENABLED`, `TTS_CACHE_REDIS_ENABLED`, `TTS_CACHE_PREFIX`, `TTS_CACHE_REDIS_TTL_SECONDS`, `TTS_CACHE_MAX_ENTRY_BYTES` (see `docs/configuration.md` / `src/env.ts`)",
  "",
  "## TTS audio cache keys (optional)",
  "- Key format: `${TTS_CACHE_PREFIX}:${sha256}` where `sha256` is a hex digest of canonical synthesis parameters (text, backend URLs, voice, cloning URL, Coqui tuning, etc.).",
  "- Value: binary blob (`VT1` magic + big-endian u16 content-type length + UTF-8 `Content-Type` string + raw audio bytes).",
  "- `SET` with TTL `TTS_CACHE_REDIS_TTL_SECONDS` (default 86400).",
  "- On Redis errors, the runtime falls back to uncached synthesis (LRU may still apply in-process).",
  "",
