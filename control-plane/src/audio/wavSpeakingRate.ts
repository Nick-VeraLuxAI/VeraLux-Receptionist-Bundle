...
export function stripSpeakingRateInstruct(instruct: string | undefined): string {
  return String(instruct || "").replace(RATE_MARK, "").trim();
}

/** Change WAV duration so 0.8x is slower and 1.2x is faster. PCM 16-bit mono only. */
export function applyWavSpeakingRate(wav: Buffer, rate: number | undefined): Buffer {
  ...
  const outFrames = Math.max(2, Math.round(frames / paced));
