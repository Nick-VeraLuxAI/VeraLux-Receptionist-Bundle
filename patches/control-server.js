...
const runtimeContract_1 = require("./runtime/runtimeContract");
const runtimePublisher_1 = require("./runtime/runtimePublisher");
const buildTenantRuntimeConfig_1 = require("./runtime/buildTenantRuntimeConfig");
const ttsPreview_1 = require("./ttsPreview");
const ttsPreviewJobs_1 = require("./ttsPreviewJobs");
...
const llmContext_1 = require("./llmContext");
const ownerAuth_1 = require("./ownerAuth");
const portalPassword_1 = require("./portalPassword");
const stripe_1 = require("./stripe");
const automations_1 = require("./automations");
const quickRepliesSuggest_1 = require("./quickRepliesSuggest");
...
function applyAdminShellCachePolicy(res) {
...
function noStoreAdminUiShell(req, res, next) {
    const p = req.path;
...
app.use((0, middleware_1.requestTimeout)(undefined, {
...
// ────────────────────────────────────────────────
const VOICE_RECORDINGS_DIR = process.env.VOICE_RECORDINGS_DIR || path_1.default.join(__dirname, "..", "public", "voice-recordings");
// Ensure voice recordings directory exists
if (!fs_1.default.existsSync(VOICE_RECORDINGS_DIR)) {
    fs_1.default.mkdirSync(VOICE_RECORDINGS_DIR, { recursive: true });
}
const voiceRecordingStorage = multer_1.default.diskStorage({
    destination: (_req, _file, cb) => {
        cb(null, VOICE_RECORDINGS_DIR);
    },
    filename: (_req, file, cb) => {
        const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
        const ext = path_1.default.extname(file.originalname) || ".wav";
...
const voiceRecordingUpload = (0, multer_1.default)({
...
const ENABLE_RUNTIME_ADMIN = parseBooleanish(process.env.ENABLE_RUNTIME_ADMIN, true);
const ALLOW_RUNTIME_SECRET_READ = parseBooleanish(process.env.ALLOW_RUNTIME_SECRET_READ, !IS_PROD);
const OPENAI_KEY_RE = /^sk-[A-Za-z0-9-_]{10,}$/;
/* ────────────────────────────────────────────────
   ✅ Admin hardening + CORS allowlist
   ──────────────────────────────────────────────── */
const ADMIN_AUTH_MODE = (process.env.ADMIN_AUTH_MODE ||
    (IS_PROD ? "jwt-only" : "hybrid")).toLowerCase();
// hybrid = allow x-admin-key OR bearer
// jwt-only = only bearer JWT (block x-admin-key) unless explicitly allowed
const ALLOW_ADMIN_API_KEY_IN_PROD = parseBooleanish(process.env.ALLOW_ADMIN_API_KEY_IN_PROD, false);
...
function isAllowedOrigin(origin) {
    ...
    if (!ADMIN_ALLOWED_ORIGINS.length)
        return !IS_PROD; // prod requires allowlist
...
function isSameOriginAsHost(req, origin) {
...
function adminCorsGuard(req, res, next) {
    ...
    if (IS_PROD && !origin && req.path.startsWith("/api/admin")) {
        middleware_1.logger.warn("Admin API request without Origin header", {
            requestId: (0, middleware_1.getRequestId)(req),
            path: req.path,
            ip: req.ip,
        });
    }
    if (IS_PROD && !originOk) {
        middleware_1.logger.warn("CORS origin rejected", {
            requestId: (0, middleware_1.getRequestId)(req),
            origin: origin || "none",
            path: req.path,
        });
    ...
    if (origin && originOk) {
        ...
        res.setHeader("Access-Control-Allow-Headers", "authorization,content-type,x-admin-key,x-tenant-id,x-active-tenant");
...
function getAdminToken(req) {
    ...
    const xAdminKey = typeof header === "string"
        ? header.trim()
...
function adminGuard(requiredRole = "viewer") {
    return async (req, res, next) => {
        try {
            const token = getAdminToken(req);
            const hasXAdminKey = typeof req.headers["x-admin-key"] === "string" ||
            ...
            const principal = await (0, auth_1.authenticateAdminKey)(token);
            if (!principal) {
            ...
            const user = await (0, db_1.upsertUserBySub)({
                idpSub: sub,
            ...
            const memberships = await (0, db_1.listMembershipsForUser)(user.id);
            ...
            if (memberships.length === 1) {
                ...
                const active = typeof activeHeader === "string"
                    ? activeHeader
                    : Array.isArray(activeHeader) && activeHeader[0]
                        ? activeHeader[0]
                        : undefined;
                const tenantIdHeader = req.headers["x-tenant-id"];
                const tenantFromHeader = typeof tenantIdHeader === "string"
                    ? tenantIdHeader.trim()
...
function optNumBody(v, min, max) {
    const n = toFiniteNumber(v);
    if (n === undefined)
        return undefined;
    return clamp(n, min, max);
}
function optIntBody(v, min, max) {
    const n = toFiniteNumber(v);
    if (n === undefined)
        return undefined;
...
function sanitizeTtsShortText(v, maxLen) {
    if (typeof v !== "string")
        return undefined;
...
function clearQwen3GenFields(u) {
...
function clearCoquiGenFields(u) {
 ...
 */
async function syncLLMContextToRuntime(tenant) {
    try {
        const existing = await (0, runtimePublisher_1.getTenantConfig)(tenant.id);
        if (!existing) {
        ...
        await (0, runtimePublisher_1.publishTenantConfig)(tenant.id, updatedConfig);
...
function respondVoiceRuntimeMoved(res) {
...
function extractTenantId(req) {
    ...
    const bodyTenant = req.body && typeof req.body.tenantId === "string"
        ? req.body.tenantId
        : undefined;
...
function extractDialedNumber(req) {
    const body = req.body || {};
    const candidates = [
        body.toNumber,
        body.calledNumber,
    ...
    candidates.push(query.toNumber || undefined, query.calledNumber || undefined, query.to || undefined);
    for (const c of candidates) {
        if (typeof c === "string" && c.trim()) {
            const normalized = (0, phone_1.normalizePhoneNumber)(c);
            if (normalized)
                return normalized;
        }
    }
...
function resolveTenant(req) {
    ...
    if (dialed) {
        const matched = tenants_1.tenants.getByNumber(dialed);
        if (matched)
            return matched;
    }
    return tenants_1.tenants.getOrCreate(tenants_1.DEFAULT_TENANT_ID);
}
...
function resolveTenantForAdmin(req) {
    ...
    if (explicitId)
        return tenants_1.tenants.getOrCreate(explicitId);
    return resolveTenant(req);
}
function getTenantForAdmin(req, res) {
    const ctx = req.ctx;
    if (ctx?.isSuperAdmin) {
    ...
    return tenants_1.tenants.getOrCreate(ctx.tenantId);
}
function ensureRuntimeAdminEnabled(res) {
...
function ensureTenantAccess(req, res, tenantId) {
    const ctx = req.ctx;
    if (!ctx) {
        res.status(403).json({ error: "tenant_context_missing" });
        return false;
...
function shouldIncludeRuntimeSecrets(req) {
...
function parsePreferredPort(value, fallback = 4000) {
    const parsed = Number(value);
    return Number.isInteger(parsed) && parsed > 0 ? parsed : fallback;
}
function isPortAvailable(port) {
    return new Promise((resolve) => {
        const tester = (0, net_1.createServer)();
...
app.post("/api/dev/receptionist-audio", (_req, res) => respondVoiceRuntimeMoved(res));
/* ────────────────────────────────────────────────
   Health
   ──────────────────────────────────────────────── */
...
app.get("/ready", async (_req, res) => {
    ...
    const dbOk = await (0, db_1.pingPool)();
    checks.db = { ok: dbOk, latencyMs: Date.now() - dbStart };
    // Check runtime Redis if enabled
    if (ENABLE_RUNTIME_ADMIN) {
        try {
            const redisHealth = await (0, runtimePublisher_1.healthcheckRedis)();
    ...
    if (process.env.REDIS_URL) {
        try {
            const rateLimitHealth = await (0, redis_2.healthcheckRedis)();
    ...
    if (!ok) {
        middleware_1.logger.warn("Health check failed", { checks });
...
app.post("/api/owner/login", async (req, res) => {
    try {
        ...
        if (emailRaw.trim() || password) {
            const emailNorm = (0, portalPassword_1.normalizePortalEmail)(emailRaw);
            if (!emailNorm || !password) {
                return res.status(400).json({ error: "email_and_password_required" });
            }
            if (!(0, portalPassword_1.isValidPortalEmailShape)(emailNorm)) {
                return res.status(400).json({ error: "invalid_email" });
            }
            const tenantId = await (0, db_1.getTenantIdByPortalEmail)(emailNorm);
            if (!tenantId) {
                return res.status(401).json({ error: "Invalid credentials" });
            }
            const row = await (0, db_1.getOwnerPortalCredentialRow)(tenantId);
            if (!row || row.emailNorm !== emailNorm) {
                return res.status(401).json({ error: "Invalid credentials" });
            }
            const valid = (0, portalPassword_1.verifyPortalPassword)(password, row.passwordHash);
            if (!valid) {
                return res.status(401).json({ error: "Invalid credentials" });
            }
            const tenant = tenants_1.tenants.getOrCreate(tenantId);
            const token = await (0, ownerAuth_1.issueOwnerJwt)({
                tenantId: tenant.id,
        ...
        if (phone.trim() && passcode) {
            ...
            const tenant = (normalized ? tenants_1.tenants.getByNumber(normalized) : undefined) ||
                tenants_1.tenants.getByNumber(stripped) ||
                tenants_1.tenants.getByNumber(digits) ||
                tenants_1.tenants.getByNumber("+" + digits) ||
                (digits.length === 10
                    ? tenants_1.tenants.getByNumber("1" + digits) ||
                        tenants_1.tenants.getByNumber("+1" + digits)
                    : undefined);
            if (!tenant) {
                return res.status(401).json({ error: "Invalid credentials" });
            }
            const valid = await (0, ownerAuth_1.verifyOwnerPasscode)(tenant.id, passcode);
            if (!valid) {
                return res.status(401).json({ error: "Invalid credentials" });
            }
            const token = await (0, ownerAuth_1.issueOwnerJwt)({
...
app.post("/api/owner/set-passcode", async (req, res) => {
    try {
        ...
        const principal = await (0, auth_1.authenticateAdminKey)(adminToken);
        ...
        await (0, ownerAuth_1.setOwnerPasscode)(tenantId, passcode);
...
app.post("/api/owner/set-portal-credentials", async (req, res) => {
    try {
        ...
        const emailNorm = (0, portalPassword_1.normalizePortalEmail)(typeof email === "string" ? email : "");
        if (!emailNorm || !(0, portalPassword_1.isValidPortalEmailShape)(emailNorm)) {
            return res.status(400).json({ error: "invalid_email" });
        }
        const pw = typeof password === "string" ? password : "";
        if (pw.length < portalPassword_1.PORTAL_PASSWORD_MIN_LEN) {
            return res.status(400).json({
                error: "password_too_short",
                minLength: portalPassword_1.PORTAL_PASSWORD_MIN_LEN,
            });
        }
        if (pw.length > portalPassword_1.PORTAL_PASSWORD_MAX_LEN) {
        ...
        const passwordHash = (0, portalPassword_1.hashPortalPassword)(pw);
        try {
            await (0, db_1.upsertOwnerPortalCredentials)({
                tenantId,
                emailNorm,
                passwordHash,
            });
...
app.post("/api/owner/change-passcode", async (req, res) => {
    try {
        ...
        const session = await (0, ownerAuth_1.verifyOwnerPortalToken)(raw);
        if (!session) {
        ...
        const result = await (0, ownerAuth_1.changeOwnerPasscodeIfValid)(session.tenantId, currentPasscode, newPasscode);
        if (!result.ok) {
...
app.post("/api/owner/change-password", async (req, res) => {
    try {
        ...
        const result = await (0, ownerAuth_1.changeOwnerPortalPasswordIfValid)(session.tenantId, currentPassword, newPassword);
        if (!result.ok) {
            if (result.error === "invalid_current") {
...
app.post("/api/admin/tenants/:tenantId/owner-passcode/change", adminGuard("admin"), async (req, res) => {
    try {
        const tenantId = req.params.tenantId?.trim();
        if (!tenantId) {
            return res.status(400).json({ error: "tenant_id_required" });
        }
        if (!ensureTenantAccess(req, res, tenantId))
            return;
        const body = req.body || {};
        const currentPasscode = typeof body.currentPasscode === "string" ? body.currentPasscode : "";
        const newPasscode = typeof body.newPasscode === "string" ? body.newPasscode : "";
        ...
        const result = await (0, ownerAuth_1.changeOwnerPasscodeIfValid)(tenantId, currentPasscode, newPasscode);
        if (!result.ok) {
            if (result.error === "invalid_current") {
                return res.status(403).json({ error: "invalid_current_passcode" });
            }
        ...
        console.error("POST /api/admin/tenants/:tenantId/owner-passcode/change error:", err);
        return res.status(500).json({ error: "change_passcode_failed" });
    }
});
app.post("/api/admin/tenants/:tenantId/owner-portal-password/change", adminGuard("admin"), async (req, res) => {
    try {
        const tenantId = req.params.tenantId?.trim();
        if (!tenantId) {
            return res.status(400).json({ error: "tenant_id_required" });
        ...
        const result = await (0, ownerAuth_1.changeOwnerPortalPasswordIfValid)(tenantId, currentPassword, newPassword);
        if (!result.ok) {
            if (result.error === "invalid_current") {
                return res.status(403).json({ error: "invalid_current_password" });
            }
        ...
        console.error("POST /api/admin/tenants/:tenantId/owner-portal-password/change error:", err);
        return res.status(500).json({ error: "change_password_failed" });
    }
});
/* ──────────────────────────────────────────────── */
const ADMIN_RATE_MAX = Number(process.env.ADMIN_RATE_MAX || 100);
const ADMIN_RATE_WINDOW_MS = Number(process.env.ADMIN_RATE_WINDOW_MS || 5 * 60 * 1000);
const ADMIN_RATE_USE_REDIS = parseBooleanish(process.env.ADMIN_RATE_USE_REDIS, false);
const ttsApiRateLimiter = (0, rateLimit_1.rateLimit)({
 ...
 */
function ttsApiRateLimitUnlessPreviewPoll(req, res, next) {
    const rel = req.path || "";
    const full = req.originalUrl || "";
    if (rel.startsWith("/preview/async") ||
...
app.use("/api/admin", adminCorsGuard, (0, rateLimit_1.rateLimit)({
    windowMs: ADMIN_RATE_WINDOW_MS,
    max: ADMIN_RATE_MAX,
    keyFn: (req) => getAdminToken(req) || req.ip || "anon",
    useRedis: ADMIN_RATE_USE_REDIS,
}), adminGuard("viewer"));
app.use("/api/tts", adminCorsGuard, ttsApiRateLimitUnlessPreviewPoll, adminGuard("admin"));
/* ────────────────────────────────────────────────
   Admin – LLM config / keys / audit
   ──────────────────────────────────────────────── */
app.get("/api/admin/auth/keys", adminGuard("admin"), async (_req, res) => {
    const keys = await (0, auth_1.listAdminKeySummaries)();
    res.json({
        keys: keys.map((k) => ({
            id: k.id,
            name: k.name,
...
app.post("/api/admin/auth/keys", adminGuard("admin"), async (req, res) => {
    ...
    const created = await (0, auth_1.createAdminKey)(name.trim(), normalizedRole);
    res.json({
        id: created.id,
        token: created.token,
        name: created.name,
...
app.delete("/api/admin/auth/keys/:id", adminGuard("admin"), async (req, res) => {
    ...
    if (!(0, validation_1.isUuid)(id))
        return res.status(400).json({ error: "invalid_id" });
    await (0, auth_1.revokeAdminKey)(id);
    res.json({ status: "ok" });
});
app.get("/api/admin/audit", adminGuard("admin"), async (req, res) => {
    try {
        const limit = Math.min(Number(req.query.limit) || 50, 200);
        const entries = await (0, db_1.listAuditLogs)(limit);
        res.json({ entries });
    }
    catch (err) {
...
app.get("/api/admin/config", async (req, res) => {
    ...
    if (!tenant)
        return;
    const hasKey = await secretStore_1.secretStore.hasSecret(tenant.id, "openai_api_key");
    const safe = tenant.config.getSafeConfig();
    res.json({
        ...safe,
        hasOpenAIApiKey: hasKey || Boolean(safe.hasOpenAIApiKey),
    });
});
app.post("/api/admin/config", (req, res) => {
    ...
    if (!tenant)
        return;
    const { provider, localUrl, openaiModel, openaiApiKey } = req.body;
    ...
    tenants_1.tenants.persistConfig(tenant.id);
    if (sanitizedKey) {
        void secretStore_1.secretStore.setSecret(tenant.id, "openai_api_key", sanitizedKey);
    }
...
app.post("/api/admin/prompts", async (req, res) => {
    ...
    if (!tenant)
        return;
    const { systemPreamble, schemaHint, policyPrompt, voicePrompt, greetingText } = req.body;
...
app.post("/api/admin/forwarding-profiles", async (req, res) => {
    ...
    const profiles = Array.isArray(raw)
        ? raw
            ...
            .map((p) => (0, llmContext_1.createForwardingProfile)({
            id: p.id,
        ...
        : [];
    const updated = tenants_1.tenants.setForwardingProfiles(tenant.id, profiles);
    if (!updated)
...
app.post("/api/admin/pricing", async (req, res) => {
    ...
    if (!tenant)
        return;
    const parsed = (0, llmContext_1.parsePricingInfo)(req.body);
    const updated = tenants_1.tenants.setPricing(tenant.id, parsed);
    if (!updated)
        return res.status(404).json({ error: "tenant_not_found" });
    // Sync LLM context to Redis for the voice runtime
    await syncLLMContextToRuntime(updated);
...
/* ────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────── */
app.get("/api/admin/subscription", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const sub = await (0, db_1.getSubscription)(tenant.id);
    if (!sub) {
...
app.post("/api/admin/subscription", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { planName, priceCents, currency, billingFrequency, status, paymentMethodBrand, paymentMethodLast4, trialEndsAt, nextBillingDate, cancelledAt, showBillingPortal, adminNotes, stripePriceId, stripeProductId, } = req.body || {};
    const sub = await (0, db_1.upsertSubscription)(tenant.id, {
...
app.delete("/api/admin/subscription", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const existing = await (0, db_1.getSubscription)(tenant.id);
    if (!existing) {
...
app.patch("/api/admin/subscription", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const existing = await (0, db_1.getSubscription)(tenant.id);
...
/* ────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────── */
app.post("/api/stripe/webhook", (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const result = await (0, stripe_1.handleStripeWebhook)(rawBody, sig);
...
app.get("/api/admin/stripe/status", (req, res) => {
    res.json({
        configured: (0, stripe_1.isStripeConfigured)(),
        publishableKey: process.env.STRIPE_PUBLISHABLE_KEY || null,
    });
});
// List available plans
app.get("/api/admin/stripe/plans", (0, middleware_1.asyncHandler)(async (req, res) => {
    const plans = await (0, stripe_1.listStripePlans)();
    res.json({ plans });
}));
// Create a new plan (admin only, creates product+price in Stripe)
app.post("/api/admin/stripe/plans", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!(0, stripe_1.isStripeConfigured)()) {
    ...
    const plan = await (0, stripe_1.createStripePlan)({ name, priceCents, currency, billingInterval });
    res.json(plan);
}));
// Delete (deactivate) a plan
app.delete("/api/admin/stripe/plans/:planId", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!(0, stripe_1.isStripeConfigured)()) {
    ...
    const deleted = await (0, stripe_1.deleteStripePlan)(planId);
    if (!deleted)
        return res.status(404).json({ error: "Plan not found" });
    res.json({ success: true });
}));
// Create a checkout session for a tenant to subscribe
app.post("/api/admin/stripe/checkout", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!(0, stripe_1.isStripeConfigured)()) {
...
app.post("/api/admin/stripe/portal", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!(0, stripe_1.isStripeConfigured)()) {
    ...
    const baseUrl = process.env.PUBLIC_BASE_URL || `http://localhost:${process.env.PORT || 4000}`;
    const session = await (0, stripe_1.createPortalSession)({
        tenantId: tenant.id,
...
app.post("/api/admin/stripe/sync", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!(0, stripe_1.isStripeConfigured)()) {
    ...
    await (0, stripe_1.syncSubscriptionFromStripe)(tenant.id, sub.stripeSubscriptionId);
    const updated = await (0, db_1.getSubscription)(tenant.id);
    res.json(updated);
}));
app.get("/api/tts/config", (req, res) => {
    ...
    if (baseCfg.defaultVoiceMode) {
        extendedCfg.defaultVoiceMode = baseCfg.defaultVoiceMode;
    }
    if (baseCfg.clonedVoice) {
        extendedCfg.clonedVoice = baseCfg.clonedVoice;
    }
    if (baseCfg.coquiXttsUrl) {
        extendedCfg.coquiXttsUrl = baseCfg.coquiXttsUrl;
    }
    if (baseCfg.kokoroUrl) {
        extendedCfg.kokoroUrl = baseCfg.kokoroUrl;
    }
    if (baseCfg.chatterboxUrl) {
        extendedCfg.chatterboxUrl = baseCfg.chatterboxUrl;
    }
    if (baseCfg.chatterboxVariant) {
        extendedCfg.chatterboxVariant = baseCfg.chatterboxVariant;
    }
    if (baseCfg.qwen3TtsUrl) {
        extendedCfg.qwen3TtsUrl = baseCfg.qwen3TtsUrl;
    }
    if (baseCfg.qwen3Instruct) {
        extendedCfg.qwen3Instruct = baseCfg.qwen3Instruct;
    }
    const bq = baseCfg;
    if (bq.qwen3DoSample !== undefined)
        extendedCfg.qwen3DoSample = bq.qwen3DoSample;
    if (bq.qwen3Temperature !== undefined)
        extendedCfg.qwen3Temperature = bq.qwen3Temperature;
    if (bq.qwen3TopP !== undefined)
        extendedCfg.qwen3TopP = bq.qwen3TopP;
    if (bq.qwen3TopK !== undefined)
        extendedCfg.qwen3TopK = bq.qwen3TopK;
    if (bq.qwen3RepetitionPenalty !== undefined)
        extendedCfg.qwen3RepetitionPenalty = bq.qwen3RepetitionPenalty;
    if (bq.qwen3MaxNewTokens !== undefined)
        extendedCfg.qwen3MaxNewTokens = bq.qwen3MaxNewTokens;
    if (bq.qwen3NonStreamingMode !== undefined)
        extendedCfg.qwen3NonStreamingMode = bq.qwen3NonStreamingMode;
    if (bq.qwen3SubtalkerDoSample !== undefined)
        extendedCfg.qwen3SubtalkerDoSample = bq.qwen3SubtalkerDoSample;
    if (bq.qwen3SubtalkerTopK !== undefined)
        extendedCfg.qwen3SubtalkerTopK = bq.qwen3SubtalkerTopK;
    if (bq.qwen3SubtalkerTopP !== undefined)
        extendedCfg.qwen3SubtalkerTopP = bq.qwen3SubtalkerTopP;
    if (bq.qwen3SubtalkerTemperature !== undefined)
        extendedCfg.qwen3SubtalkerTemperature = bq.qwen3SubtalkerTemperature;
    if (bq.qwen3Streaming !== undefined)
        extendedCfg.qwen3Streaming = bq.qwen3Streaming;
    if (bq.coquiTemperature !== undefined)
        extendedCfg.coquiTemperature = bq.coquiTemperature;
    if (bq.coquiLengthPenalty !== undefined)
        extendedCfg.coquiLengthPenalty = bq.coquiLengthPenalty;
    if (bq.coquiRepetitionPenalty !== undefined)
        extendedCfg.coquiRepetitionPenalty = bq.coquiRepetitionPenalty;
    if (bq.coquiTopK !== undefined)
        extendedCfg.coquiTopK = bq.coquiTopK;
    if (bq.coquiTopP !== undefined)
        extendedCfg.coquiTopP = bq.coquiTopP;
    if (bq.coquiSpeed !== undefined)
        extendedCfg.coquiSpeed = bq.coquiSpeed;
    if (bq.coquiSplitSentences !== undefined)
        extendedCfg.coquiSplitSentences = bq.coquiSplitSentences;
...
app.post("/api/tts/config", async (req, res) => {
    ...
    if (!tenant)
        return;
    const body = req.body;
    const { xttsUrl, coquiXttsUrl, kokoroUrl, chatterboxUrl, chatterboxVariant, qwen3TtsUrl, qwen3Instruct, voiceId, language, rate, preset, ttsMode, defaultVoiceMode, clonedVoice, } = body;
    ...
    const safeRate = safeRateRaw !== undefined ? clamp(safeRateRaw, 0.8, 1.2) : undefined;
    ...
    const validChatterboxVariant = (v) => v === "turbo" || v === "standard" || v === "multilingual";
    ...
    if (ttsMode === "coqui_xtts") {
        ...
        if ("qwen3TopP" in body) {
            ...
            configUpdate.qwen3TopP = v === null || v === undefined ? undefined : optNumBody(v, 0, 1);
        }
        ...
        if ("qwen3MaxNewTokens" in body) {
            ...
            configUpdate.qwen3MaxNewTokens =
                v === null || v === undefined ? undefined : optIntBody(v, 1, 32768);
        }
        ...
        if ("qwen3SubtalkerTopP" in body) {
            ...
            configUpdate.qwen3SubtalkerTopP =
                v === null || v === undefined ? undefined : optNumBody(v, 0, 1);
        }
    ...
    if (ENABLE_RUNTIME_ADMIN) {
        try {
            const existing = await (0, runtimePublisher_1.getTenantConfig)(tenant.id);
            const parsed = (0, buildTenantRuntimeConfig_1.buildTenantRuntimeConfig)(tenant, existing);
            await (0, runtimePublisher_1.publishTenantConfig)(tenant.id, parsed);
            runtimePublish = { ok: true };
        }
        catch (err) {
            if (err instanceof buildTenantRuntimeConfig_1.BuildRuntimeConfigError) {
                runtimePublish = {
                    ok: false,
                    error: err.code,
                    message: err.message,
...
app.post("/api/tts/preview", async (req, res) => {
    ...
    try {
        const raw = req.body;
        const text = (0, ttsPreview_1.resolvePreviewText)(raw?.text);
        ...
        const { body: audio } = await (0, ttsPreview_1.synthesizeTtsPreview)(cfg, text);
        res.end(audio);
    }
    catch (err) {
        if (responseStarted) {
            middleware_1.logger.error("POST /api/tts/preview failed after headers sent", {
                err,
                tenantId: tenant.id,
            });
            res.destroy();
        ...
        middleware_1.logger.error("POST /api/tts/preview failed", { err, tenantId: tenant.id });
...
app.post("/api/tts/preview/async", async (req, res) => {
    ...
    try {
        ...
        const text = (0, ttsPreview_1.resolvePreviewText)(raw?.text);
        const cfg = tenant.config.getTtsConfig();
        const id = await (0, ttsPreviewJobs_1.createPreviewJob)(tenant.id, cfg, text);
...
app.get("/api/tts/preview/async/:id", async (req, res) => {
    ...
    if (!tenant)
        return;
    const { id } = req.params;
    if (!id || !(0, validation_1.isUuid)(id)) {
        return res.status(400).json({ error: "invalid_id" });
    }
    const result = await (0, ttsPreviewJobs_1.pollPreviewJob)(id, tenant.id);
    switch (result.kind) {
        case "not_found":
            return res.status(200).json({
                status: "failed",
                error: "not_found",
                message: "Preview job not found (expired or created on another server). With multiple control-plane replicas, REDIS_URL must be set so jobs are shared in Redis.",
            });
        case "forbidden":
            return res.status(200).json({
                status: "failed",
        ...
        case "error": {
            const msg = result.code === "tts_url_missing"
...
app.get("/api/admin/telephony/secret", (req, res) => {
    ...
    if (!tenant)
        return;
    void secretStore_1.secretStore
...
// ────────────────────────────────────────────────
app.post("/api/admin/voice-recordings", voiceRecordingUpload.single("audio"), (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    if (!req.file) {
    ...
    const relativePath = path_1.default.relative(path_1.default.join(__dirname, "..", "public"), req.file.path);
...
app.get("/api/admin/health", (req, res) => {
    ...
    const hasOpenAIApiKey = (() => {
        const raw = cfg.openaiApiKey || process.env.OPENAI_API_KEY || "";
        const t = String(raw).trim();
        if (!t)
            return false;
        if (/^CHANGE_ME/i.test(t) || /^PASTE_/i.test(t) || /^sk-test/i.test(t) || t === "EMPTY" || t === "your-api-key-here")
            return false;
        return true;
    })();
    const localLlmConfigured = Boolean((cfg.localUrl || process.env.LOCAL_LLM_URL || "").trim());
    const llmStatus = cfg.provider === "openai"
        ? hasOpenAIApiKey
            ? "ready"
            : "missing_api_key"
        : localLlmConfigured
            ? "configured"
            : "defaulting";
    const activeCallsByTenant = {};
    let activeCallsGlobal = 0;
    for (const meta of tenants_1.tenants.listMetas()) {
        const t = tenants_1.tenants.getOrCreate(meta.id);
    ...
    res.json({
        ...
        tts: {
            status: tts.xttsUrl || tts.coquiXttsUrl || tts.kokoroUrl || tts.chatterboxUrl || tts.qwen3TtsUrl
                ? "configured"
                : "missing",
            xttsUrl: tts.xttsUrl,
...
app.post("/api/runtime/analytics", adminGuard("admin"), (req, res) => {
    const { tenantId, event, text } = req.body;
    if (!tenantId || typeof tenantId !== "string") {
        return res.status(400).json({ error: "tenant_id_required" });
    }
    const tenant = tenants_1.tenants.getOrCreate(tenantId);
...
app.post("/api/runtime/calls", adminGuard("admin"), (req, res) => {
    const { tenantId, callId, action, callState } = req.body;
    if (!tenantId || typeof tenantId !== "string") {
        return res.status(400).json({ error: "tenant_id_required" });
    }
    const tenant = tenants_1.tenants.getOrCreate(tenantId);
    ...
    if (action === "update" && callId) {
        ...
        const updated = tenant.calls.save({
            ...existing,
            ...(callState?.stage ? { stage: callState.stage } : {}),
            ...(callState?.lead ? { lead: { ...existing.lead, ...callState.lead } } : {}),
            ...(callState?.history ? { history: callState.history } : {}),
        });
    ...
    if (action === "end" && callId) {
        ...
        // Always fire workflow event bus on hangup — runtime sends Telnyx call_control_id
        // which often won't match in-memory start UUID; still book from body history/transcript.
        const historyForWorkflow = Array.isArray(req.body?.callState?.history)
            ? req.body.callState.history
            : (endingCall ? endingCall.history : []);
        const workflowEvent = {
            type: "call_ended",
            tenantId,
            callId,
            callerId: endingCall?.callerId ?? req.body?.callState?.callerId,
            durationMs: endingCall?.createdAt
                ? Date.now() - endingCall.createdAt
                : undefined,
            turns: historyForWorkflow,
            transcript: req.body.transcript,
            lead: endingCall?.lead ?? req.body?.callState?.lead ?? {},
            timestamp: new Date().toISOString(),
        };
        (0, automations_1.handleCallEnded)(workflowEvent).catch(err => {
...
app.get("/api/admin/tenants", (_req, res) => {
    res.json({ tenants: tenants_1.tenants.listMetas() });
});
/** Client portal readiness: legacy passcode and/or email login. */
app.get("/api/admin/tenants/:tenantId/owner-portal-status", (0, middleware_1.asyncHandler)(async (req, res) => {
    ...
    const [hash, cred] = await Promise.all([
        (0, db_1.getOwnerPasscodeHash)(tenantId),
        (0, db_1.getOwnerPortalCredentialRow)(tenantId),
    ]);
    res.json({
...
app.post("/api/admin/tenants", (req, res) => {
    const { id, name, numbers, businessNumber } = req.body;
    ...
    const updated = tenants_1.tenants.upsertMeta(id.trim(), {
...
/* ────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────── */
const telnyx = __importStar(require("./telnyx"));
...
app.get("/api/admin/telnyx/available", async (req, res) => {
    ...
    try {
        ...
        res.json({
            numbers,
            available: numbers.map((number) => ({
                phone_number: number.phone_number,
                region: {
                    city: (number.region_information || []).find((region) => ["rate_center", "location", "locality"].includes(region.region_type))?.region_name || null,
                    state: (number.region_information || []).find((region) => ["state", "administrative_area"].includes(region.region_type))?.region_name || null,
                    country: (number.region_information || []).find((region) => ["country_code", "country"].includes(region.region_type))?.region_name || null,
                },
                features: (number.features || []).map((feature) => (typeof feature === "string" ? feature : feature?.name)).filter(Boolean),
...
/* ────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────── */
app.post("/api/admin/runtime/tenants/:tenantId/config", adminGuard("admin"), async (req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    const tenantId = req.params.tenantId?.trim();
    if (!tenantId)
        return res.status(400).json({ error: "tenant_id_required" });
    ...
    let parsed;
    try {
        parsed = (0, runtimeContract_1.parseRuntimeTenantConfig)(req.body);
    }
    catch (err) {
        return res.status(400).json({
    ...
    try {
        await (0, runtimePublisher_1.publishTenantConfig)(tenantId, parsed);
    }
    catch (err) {
 ...
 */
app.post("/api/admin/runtime/tenants/:tenantId/publish-from-tenant", adminGuard("admin"), async (req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    const tenantId = req.params.tenantId?.trim();
    if (!tenantId)
    ...
    let existing;
    try {
        existing = await (0, runtimePublisher_1.getTenantConfig)(tenantId);
    }
    catch (err) {
    ...
    let parsed;
    try {
        parsed = (0, buildTenantRuntimeConfig_1.buildTenantRuntimeConfig)(tenant, existing);
    }
    catch (err) {
        if (err instanceof buildTenantRuntimeConfig_1.BuildRuntimeConfigError) {
            return res.status(400).json({
...
app.get("/api/admin/runtime/tenants/:tenantId/config", async (req, res) => {
    ...
    if (!ensureTenantAccess(req, res, tenantId))
        return;
    try {
        const config = await (0, runtimePublisher_1.getTenantConfig)(tenantId);
        if (!config)
            return res.status(404).json({ error: "runtime_config_not_found" });
        const includeSecrets = shouldIncludeRuntimeSecrets(req);
        return res.json({
...
const quickRepliesPutBodySchema = zod_1.z.object({
    quickReplies: zod_1.z.array(runtimeContract_1.quickReplyIntentSchema).max(200),
});
/**
...
app.post("/api/admin/quick-replies/suggest", adminGuard("admin"), async (req, res) => {
    ...
    if (!apiKey || apiKey === "CHANGE_ME") {
        return res.status(503).json({
            ...
            message: "Set OPENAI_API_KEY for the control plane (not CHANGE_ME) to use AI quick-reply suggestions.",
        });
    }
    const parsed = quickRepliesSuggest_1.quickRepliesSuggestBodySchema.safeParse(req.body);
    if (!parsed.success) {
        return res.status(400).json({
            error: "invalid_body",
    ...
    try {
        const { quickReplies, dropped } = await (0, quickRepliesSuggest_1.suggestQuickRepliesWithOpenAI)(apiKey, model, parsed.data);
...
app.put("/api/admin/runtime/tenants/:tenantId/quick-replies", adminGuard("admin"), async (req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    const tenantId = req.params.tenantId?.trim();
    if (!tenantId)
    ...
    try {
        const existing = await (0, runtimePublisher_1.getTenantConfig)(tenantId);
        if (!existing) {
            return res.status(404).json({
                error: "runtime_config_not_found",
                message: "Publish tenant config to Redis first (admin: Step 3 Voice → Publish to voice runtime, or POST /api/admin/runtime/tenants/:tenantId/publish-from-tenant), then quick replies can be saved.",
            });
        }
        const merged = (0, runtimeContract_1.parseRuntimeTenantConfig)({
            ...existing,
            quickReplies: parsedBody.data.quickReplies,
        });
        await (0, runtimePublisher_1.publishTenantConfig)(tenantId, merged);
        return res.json({
            status: "ok",
...
app.post("/api/admin/runtime/dids/map", adminGuard("admin"), async (req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    const { didE164, tenantId } = req.body;
    if (!didE164 || typeof didE164 !== "string") {
        return res.status(400).json({ error: "did_required" });
    }
    let did;
    try {
        did = (0, runtimeContract_1.normalizeE164)(didE164);
    }
    catch (err) {
        const message = String(err);
        const error = message.includes("did_empty") ? "did_required" : "invalid_did_e164";
    ...
    if (!ensureTenantAccess(req, res, targetTenantId))
        return;
    try {
        // Postgres `tenant_numbers` is canonical; setTenantNumbers syncs Redis.
        await (0, db_1.addTenantNumberIfMissing)(targetTenantId, did);
...
app.post("/api/admin/runtime/dids/unmap", adminGuard("admin"), async (req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    const { didE164 } = req.body;
    if (!didE164 || typeof didE164 !== "string") {
        return res.status(400).json({ error: "did_required" });
    }
    let did;
    ...
    try {
        const mappedTenant = await (0, db_1.findTenantIdByInboundNumberE164)(did);
        if (!mappedTenant) {
            return res.status(404).json({ error: "did_unmapped" });
        }
        if (!ensureTenantAccess(req, res, mappedTenant))
            return;
        const nums = await (0, db_1.getTenantNumbers)(mappedTenant);
        const next = nums.filter((n) => {
            try {
                return (0, runtimeContract_1.normalizeE164)(n) !== did;
            }
            catch {
                return true;
            }
        });
        await (0, db_1.setTenantNumbers)(mappedTenant, next);
...
app.get("/api/admin/runtime/dids/:didE164", async (req, res) => {
    ...
    let did;
    try {
        did = (0, runtimeContract_1.normalizeE164)(didParam);
    }
    catch (err) {
        const message = String(err);
        const error = message.includes("did_empty") ? "did_required" : "invalid_did_e164";
    ...
    try {
        const tenantId = await (0, db_1.findTenantIdByInboundNumberE164)(did);
        if (!tenantId)
            return res.status(404).json({ error: "did_unmapped" });
        if (!ensureTenantAccess(req, res, tenantId))
            return;
...
app.get("/api/admin/runtime/health", async (_req, res) => {
    if (!ensureRuntimeAdminEnabled(res))
        return;
    try {
        const health = await (0, runtimePublisher_1.healthcheckRedis)();
...
app.get("/v1/calls/:callControlId/voice", adminCorsGuard, adminGuard("viewer"), async (req, res) => {
...
app.post("/v1/calls/:callControlId/voice", adminCorsGuard, adminGuard("admin"), async (req, res) => {
    const callControlId = req.params.callControlId?.trim();
    if (!callControlId) {
        return res.status(400).json({ error: "call_control_id_required" });
    }
    const { mode, speakerWavUrl } = req.body;
    // Validate mode
    ...
    try {
        ...
        const payload = { mode };
        if (speakerWavUrl)
            payload.speakerWavUrl = speakerWavUrl;
        const response = await fetch(runtimeUrl, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
...
app.get("/api/admin/workflows", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const workflows = await (0, automations_1.listWorkflows)(tenant.id);
    res.json({ workflows });
}));
// Create a workflow
app.post("/api/admin/workflows", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    ...
    const createdBy = req.adminRole === "admin" ? "admin" : "owner";
    const wf = await (0, automations_1.createWorkflow)({
        tenantId: tenant.id,
        name,
        triggerType,
...
// Update a workflow
app.put("/api/admin/workflows/:id", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { id } = req.params;
    const existing = await (0, automations_1.getWorkflow)(id);
    if (!existing || existing.tenantId !== tenant.id) {
        return res.status(404).json({ error: "Workflow not found" });
    }
    const { name, enabled, triggerType, triggerConfig, steps, adminLocked } = req.body || {};
    const updated = await (0, automations_1.updateWorkflow)(id, {
        name, enabled, triggerType, triggerConfig, steps, adminLocked,
    });
    res.json(updated);
}));
// Delete a workflow
app.delete("/api/admin/workflows/:id", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { id } = req.params;
    ...
    await (0, automations_1.deleteWorkflow)(id);
    res.json({ success: true });
}));
// Dry-run / test a workflow
app.post("/api/admin/workflows/:id/test", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { id } = req.params;
    const workflow = await (0, automations_1.getWorkflow)(id);
    ...
    const result = await (0, automations_1.dryRunPipeline)(workflow, sampleEvent);
    res.json(result);
}));
// Workflow execution history
app.get("/api/admin/workflow-runs", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const limit = parseInt(req.query.limit) || 50;
    const runs = await (0, automations_1.listRuns)(tenant.id, limit);
    res.json({ runs });
}));
// List leads
app.get("/api/admin/leads", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const limit = parseInt(req.query.limit) || 100;
    const leads = await (0, automations_1.listLeads)(tenant.id, limit);
    res.json({ leads });
}));
// Delete a lead
app.delete("/api/admin/leads/:id", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { id } = req.params;
    const deleted = await (0, automations_1.deleteLead)(id);
    if (!deleted)
        return res.status(404).json({ error: "Lead not found" });
    res.json({ success: true });
}));
// Get workflow settings for tenant
app.get("/api/admin/workflows/settings", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const settings = await (0, automations_1.getWorkflowSettings)(tenant.id);
    res.json(settings);
}));
// Update workflow settings for tenant
app.patch("/api/admin/workflows/settings", (0, middleware_1.asyncHandler)(async (req, res) => {
    const tenant = getTenantForAdmin(req, res);
    if (!tenant)
        return;
    const { ownerCanEdit } = req.body || {};
    const settings = await (0, automations_1.updateWorkflowSettings)(tenant.id, {
...
app.post("/api/calls/:callId/message", (_req, res) => respondVoiceRuntimeMoved(res));
app.post("/api/calls/:callId/end", (_req, res) => respondVoiceRuntimeMoved(res));
app.post("/api/telnyx/call-control", (_req, res) => respondVoiceRuntimeMoved(res));
app.get("/api/telnyx/audio/:id.wav", (_req, res) => respondVoiceRuntimeMoved(res));
/* ────────────────────────────────────────────────
   Admin UI shell
   ──────────────────────────────────────────────── */
...
app.get("/owner", (_req, res) => {
    applyAdminShellCachePolicy(res);
    res.sendFile(path_1.default.join(__dirname, "..", "public", "owner.html"));
});
app.get("/portal", (req, res) => {
    applyAdminShellCachePolicy(res);
    res.sendFile(path_1.default.join(__dirname, "..", "public", "portal.html"));
});
/* ────────────────────────────────────────────────
   Bootstrap
   ──────────────────────────────────────────────── */
...
function isStrongSecret(s) {
    ...
    if (v.length < 24)
        return false;
    if (v.includes("dev-secret") || v.includes("change-me"))
        return false;
    return true;
}
// ────────────────────────────────────────────────
...
async function start() {
    ...
    try {
        (0, automations_1.initAutomationEngine)();
    }
    catch (err) {
    ...
    if (ENABLE_RUNTIME_ADMIN) {
        try {
            (0, runtimePublisher_1.assertRuntimeRedisConfigured)();
        }
        catch (err) {
    ...
    try {
        ...
        httpServer = app.listen(port, () => {
            console.log(`VeraLux Receptionist server listening on port ${port}${port !== preferredPort ? ` (preferred ${preferredPort} unavailable)` : ""}`);
        });
    }
    catch (err) {
...
function shutdown(signal) {
    ...
    try {
        httpServer?.close(async () => {
            try {
                (0, automations_1.shutdownAutomationEngine)();
                await (0, db_1.closePool)();
                if (ENABLE_RUNTIME_ADMIN)
                    await (0, runtimePublisher_1.closeRuntimeRedis)();
                if (ADMIN_RATE_USE_REDIS)
