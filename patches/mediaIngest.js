...
const audioInvariantReport_1 = require("../observability/audioInvariantReport"); // VERA_DEMO_SHOP_AUDIOINV_20260907 observe-only
...
function resolveDecodeJoinCrossfadeSamples() {
...
function resolveDecodeJoinMinDelta() {
...
function mergePendingPcmWithDecoded(pending, pcm16, crossfade, minDelta) {
...
function normalizeCodec(value) {
    ...
    if (normalized === 'AMRWB' || normalized === 'AMR_WB')
        return 'AMR-WB';
    if ((0, streamCodec_1.isL16Codec)(normalized))
        return 'L16';
    return normalized;
}
...
class MediaIngest {
    ...
    handleMessage(message) {
        ...
        if (event === 'mark') {
            const mark = message.mark && typeof message.mark === 'object' ? message.mark : {};
        ...
        if (buffer.length < minDecodedLen) {
            ...
            (0, audioInvariantReport_1.recordDecodeFailure)(this.callControlId, currentCodec, buffer.length);
        ...
        if (shouldCommitSeq && this.activeStreamId && typeof seqNum === 'number' && Number.isFinite(seqNum)) {
            ...
            if (prevSeq >= 0 && seqNum > prevSeq + 1) {
                ...
                (0, metrics_1.incMediaInboundSeqGapFrames)(gap);
                const warnMin = env_1.env.MEDIA_SEQ_GAP_LOG_MIN;
                (0, audioInvariantReport_1.recordSeqGap)(this.callControlId, gap);
                if (gap >= warnMin) {
    ...
    handleStartEvent(message) {
        ...
        if (!this.mediaCodecLogged) {
            ...
            const l16InUse = (0, streamCodec_1.isL16Codec)(normalizedEncoding) && (this.mediaSampleRate ?? 16000) === 16000;
            if (!l16InUse) {
                log_1.log.error({
    ...
    isCodecSupported(codec) {
        if (codec === 'AMR-WB')
            return { supported: false, reason: 'amrwb_stream_fallback_disabled' };
        if (!this.acceptCodecs.has(codec) && !(0, streamCodec_1.isL16Codec)(codec)) {
    ...
    handleEncodedPayload(buffer, timestamp, seq, payloadSource) {
        if (!this.mediaEncoding) {
            log_1.log.warn({
                ...
                call_control_id: this.callControlId,
                assumed_codec: streamCodec_1.TELNYX_WS_STREAM_CODEC,
        ...
        const fosIn = (0, audioForensics_1.getForensicsSession)(this.callControlId);
        if (fosIn) {
            const seqPart = typeof seq === 'number' && Number.isFinite(seq)
    ...
    async decodeAndEmit(buffer, encoding, timestamp, seq, payloadSource) {
        ...
        if (!decodeResult) {
            ...
            if (!amrwbBuffering) {
                (0, audioInvariantReport_1.recordDecodeFailure)(this.callControlId, encoding, buffer.length);
                this.checkHealth(encoding);
            }
            return;
        }
        ...
        decodeOk = true;
        (0, audioInvariantReport_1.recordDecodeOk)(this.callControlId);
        const sampleRateHz = decodeResult.sampleRateHz;
        const fosDec = (0, audioForensics_1.getForensicsSession)(this.callControlId);
        if (fosDec && pcm16.length > 0) {
            ...
            void fosDec
                .writeBinary(`audio/002_decoded_pcm_${idx}.wav`, (0, audioForensics_1.encodePcm16MonoWav)(pcmBufFull, sampleRateHz))
                .catch(() => undefined);
        ...
        let combined = pcm16;
        if (this.pendingPcm && this.pendingPcm.length > 0) {
            combined = mergePendingPcmWithDecoded(this.pendingPcm, pcm16, this.decodeJoinCrossfadeSamples, this.decodeJoinMinDeltaInt16);
            this.pendingPcm = undefined;
            this.pendingPcmSampleRateHz = undefined;
        }
        ...
        while (combined.length - offset >= emitSamples) {
            ...
            const fosEmit = (0, audioForensics_1.getForensicsSession)(this.callControlId);
            if (fosEmit && slice.length > 0) {
                ...
                if (fosEmit.mediaIngestChunkIndex <= env_1.env.AUDIO_FORENSICS_MAX_EMIT_FRAMES) {
                    const pcmSliceBuf = Buffer.from(slice.buffer, slice.byteOffset, slice.byteLength);
                    void fosEmit
                        .writeBinary(`audio/003_emit_frame_${String(fosEmit.mediaIngestChunkIndex).padStart(4, '0')}.wav`, (0, audioForensics_1.encodePcm16MonoWav)(pcmSliceBuf, sampleRateHz))
                        .catch(() => undefined);
                }
                (0, audioForensics_1.forensicsTimeline)(this.callControlId, {
    ...
    async handleUnhealthy(reason, codec) {
        ...
        const requestedCodec = streamCodec_1.TELNYX_WS_STREAM_CODEC;
        ...
        try {
            ...
            if (ok && requestedCodec === streamCodec_1.TELNYX_WS_STREAM_CODEC) {
                this.pinnedCodec = streamCodec_1.TELNYX_WS_STREAM_CODEC;
            }
            if (!ok) {
                log_1.log.warn({
