...
const voiceControlAuth_1 = require("./security/voiceControlAuth");
...
function normalizeCodec(value) {
    ...
    if (!normalized)
        return streamCodec_1.TELNYX_WS_STREAM_CODEC;
...
function logTtsBytesReady(id, audio, contentType, context, source = 'kokoro') {
...
function attachMediaWebSocketServer(server, sessionManager) {
    ...
    const acceptCodecs = new Set(['L16', 'LINEAR16', 'PCM16', 'PCM16LE']);
    ...
    wss.on('connection', (ws, request) => {
        ...
        void (0, audioForensics_1.ensureForensicsSession)(callControlId, sessionManager.getTenantRuntimeConfigForCall(callControlId) ?? null).then((sess) => {
            if (!sess)
                return;
        ...
        const ingest = new mediaIngest_1.MediaIngest({
            ...
            onRestartStreaming: async (codec, reason) => {
                ...
                await telnyx.startStreaming(callControlId, streamUrl, {
                    streamCodec: streamCodec_1.TELNYX_WS_STREAM_CODEC,
        ...
        log_1.log.info({
            ...
            url: request.url,
            requested_stream_codec: streamCodec_1.TELNYX_WS_STREAM_CODEC,
...
async function ensureGreetingAsset() {
    ...
    try {
        await fs_1.default.promises.rm(greetingPath, { force: true });
    }
    catch (error) {
        const code = error.code;
        if (code && code !== 'ENOENT')
            log_1.log.warn({ err: error, path: greetingPath }, 'greeting asset remove failed');
    }
    const voice = env_1.env.TTS_MODE === 'coqui_xtts'
        ? (env_1.env.COQUI_VOICE_ID ?? 'en_sample')
        : env_1.env.TTS_MODE === 'chatterbox_http'
            ? (env_1.env.CHATTERBOX_VOICE_ID ?? 'chatterbox')
            : env_1.env.TTS_MODE === 'qwen3_tts_http'
                ? (env_1.env.QWEN3_TTS_SPEAKER ?? 'Ryan')
                : (env_1.env.KOKORO_VOICE_ID ?? 'af_bella');
    try {
        const greetingText = env_1.env.GREETING_TEXT ?? 'Hi! Thanks for calling. How can I help you today?';
        const result = await (0, tts_1.synthesizeSpeech)({
            text: greetingText,
            voice,
            format: 'wav',
        });
        const ttsSource = env_1.env.TTS_MODE === 'coqui_xtts'
            ? 'coqui_xtts'
            : env_1.env.TTS_MODE === 'chatterbox_http'
                ? 'chatterbox'
                : env_1.env.TTS_MODE === 'qwen3_tts_http'
...
const GREETING_RETRY_MS = 60000;
function scheduleGreetingWithRetry() {
    void (async () => {
        await ensureGreetingAsset();
        try {
            await fs_1.default.promises.stat(path_1.default.join(env_1.env.AUDIO_STORAGE_DIR, 'greeting.wav'));
            return;
        }
        catch {
            log_1.log.info({ retryInMs: GREETING_RETRY_MS }, 'greeting asset missing, will retry');
...
function buildServer() {
    ...
    if (env_1.env.STT_DEBUG_DIR) {
        fs_1.default.mkdirSync(env_1.env.STT_DEBUG_DIR, { recursive: true });
        log_1.log.debug({ dir: env_1.env.STT_DEBUG_DIR }, 'STT debug dir ensured');
    }
    if (env_1.env.AMRWB_DEBUG_DIR) {
        fs_1.default.mkdirSync(env_1.env.AMRWB_DEBUG_DIR, { recursive: true });
        log_1.log.debug({ dir: env_1.env.AMRWB_DEBUG_DIR }, 'AMR-WB debug dir ensured');
    }
    if (env_1.env.CALL_TRANSCRIPT_DIR) {
        fs_1.default.mkdirSync(env_1.env.CALL_TRANSCRIPT_DIR.trim(), { recursive: true });
        log_1.log.debug({ dir: env_1.env.CALL_TRANSCRIPT_DIR }, 'call transcript dir ensured');
    }
    log_1.log.info({
     ...
     */
    app.get('/v1/calls/:callControlId/voice', voiceControlAuth_1.voiceControlGuard, (req, res) => {
     ...
     */
    app.post('/v1/calls/:callControlId/voice', voiceControlAuth_1.voiceControlGuard, (req, res) => {
        const { callControlId } = req.params;
        const body = req.body;
        // Validate mode
        ...
        const speakerWavUrl = typeof body?.speakerWavUrl === 'string' && body.speakerWavUrl.trim()
    ...
    log_1.log.info({ routes: ['/v1/calls/:callControlId/voice'] }, 'call management API configured');
