...
# Online installer API (override for white-label / self-hosted installer backend)
INSTALLER_CONFIG_API_URL="${INSTALLER_CONFIG_API_URL:-https://api.veralux.ai/api/v1/installer/config}"
API_URL="$INSTALLER_CONFIG_API_URL"
INSTALLER_SIGNUP_URL="${INSTALLER_SIGNUP_URL:-https://veralux.ai/signup}"
...
# Version & Registry (pin VERSION to a tag that exists in REGISTRY; avoid "latest" for reproducible pulls)
VERSION=0.1.0
...
# Runtime URLs (for real PSTN, set PUBLIC_BASE_URL / AUDIO_PUBLIC_BASE_URL to your HTTPS tunnel or domain)
PUBLIC_BASE_URL=http://localhost:4001
AUDIO_PUBLIC_BASE_URL=http://localhost:4001/audio
TTS_MODE=coqui_xtts
...
$( [[ -n "${cloudflare_token// /}" ]] && echo "CLOUDFLARED_TAG=2026.2.0" || true )
ENVFILE
}
...
main() {
    ...
    if [[ "$SETUP_METHOD" == "Online"* ]]; then
        ...
        if [[ -z "$SUCCESS" ]]; then
            ...
            echo "Don't have an account? Sign up at $INSTALLER_SIGNUP_URL"
            exit 1
        fi
    ...
    echo "    ./up                 - Start stack (same as ./deploy.sh up; use this after reboot)"
